...
def main():
    ...
    print(f"\n{'=' * 50}")
    print("Feature Selection 2: SelectKBest + f_regression")
    print("=" * 50)
    indices_sklearn, scores_sklearn = select_features_sklearn(X_train, y_train, k=5)
    print(f"Selected indices: {indices_sklearn}")
    print(f"Feature names:    {[feature_names[i] for i in indices_sklearn]}")
    print(f"F-scores:         {scores_sklearn}")
    ...
    # --- 6. Random Forest (SelectKBest features) ---
    print(f"\n{'=' * 50}")
    print("Training: Random Forest (SelectKBest features)")
    print("=" * 50)
    rf_sk_model = train_random_forest(X_train_scaled[:, indices_sklearn], y_train)
    y_pred_rf_sk = rf_sk_model.predict(X_test_scaled[:, indices_sklearn])
    mse_rf_sklearn = compute_mse(y_test, y_pred_rf_sk)
    print(f"RF (SelectKBest) MSE: {mse_rf_sklearn:.4f}")
    ...
    print(f"{'RF (SelectKBest 5)':<35} {mse_rf_sklearn:<15.4f} {np.sqrt(mse_rf_sklearn):<15.4f}")
    print("=" * 65)

    best_basic_mse = min(mse_rf_all, mse_rf_numpy, mse_rf_sklearn)
    ...
    with open("output/baseline_results.txt", "w", encoding="utf-8") as f:
        ...
        f.write(f"RF (SelectKBest 5)         MSE: {mse_rf_sklearn:.6f}\n")
        f.write("=" * 50 + "\n")
